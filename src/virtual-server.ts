    ...
    */
    constructor( proxy: Proxy, serverConfig: IConfig ) {
        ...
        if ( serverConfig.ssl ) {
            ...
            const caChain = [ fs.readFileSync( serverConfig.sslIntermediate! ), fs.readFileSync( serverConfig.sslRoot! ) ];
            const privateKey = serverConfig.sslKey ? fs.readFileSync( serverConfig.sslKey ) : null;
            const theCert = serverConfig.sslCert ? fs.readFileSync( serverConfig.sslCert ) : null;
    ...
    onSocketRequest( req: http.ServerRequest, socket, head ) {
        ...
        const fullURI: string = `${( ( <any>req.connection ).encrypted ? 'wss' : 'ws' )}://${req.headers.host}${req.url}`;

        // You can define here your custom logic to handle the request
        // and then proxy the request.
        for ( let i = 0, l = cfg.routes.length; i < l; i++ ) {
    ...
    */
    onServerRequest( req: http.ServerRequest, res: http.ServerResponse ) {
        if ( req.headers && req.headers.host ) {
            const cfg = this._cfg;
            const proxy = this._proxy;
            const fullURI: string = `${( ( <any>req.connection ).encrypted ? 'https' : 'http' )}://${req.headers.host}${req.url}`;

            // You can define here your custom logic to handle the request
            // and then proxy the request.
            for ( let i = 0, l = this._cfg.routes.length; i < l; i++ ) {
